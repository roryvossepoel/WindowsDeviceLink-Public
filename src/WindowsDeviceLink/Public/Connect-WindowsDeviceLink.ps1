function Connect-WindowsDeviceLink {
    ...
    param(
        ...
        [Parameter(Mandatory, ParameterSetName = 'AccessToken')][ValidateNotNull()][securestring]$AccessToken,
        [Parameter(Mandatory, ParameterSetName = 'Certificate')][ValidateNotNull()][System.Security.Cryptography.X509Certificates.X509Certificate2]$Certificate,
        [Parameter(Mandatory, ParameterSetName = 'CertificateThumbprint')][ValidateNotNullOrEmpty()][string]$CertificateThumbprint,
        [Parameter(Mandatory, ParameterSetName = 'CertificateSubjectName')][ValidateNotNullOrEmpty()][string]$CertificateSubjectName,
        [Parameter(ParameterSetName = 'Certificate')][Parameter(ParameterSetName = 'CertificateThumbprint')][Parameter(ParameterSetName = 'CertificateSubjectName')][bool]$SendCertificateChain = $false,
        [Parameter(Mandatory, ParameterSetName = 'ClientSecret')][ValidateNotNull()][securestring]$ClientSecret,
        [Parameter(Mandatory, ParameterSetName = 'ManagedIdentity')][Alias('ManagedIdentity')][switch]$Identity,
        [Parameter(Mandatory, ParameterSetName = 'EnvironmentVariable')][switch]$EnvironmentVariable,
    ...
    if (-not (Get-Command Connect-MgGraph -ErrorAction SilentlyContinue)) { throw 'Microsoft.Graph.Authentication is unavailable after dependency initialization.' }

    $parameters = @{ Environment = $Environment; ClientTimeout = $ClientTimeout; NoWelcome = $true }
    switch ($PSCmdlet.ParameterSetName) {
        'Interactive' {
            if($TenantId){$parameters.TenantId=$TenantId};$parameters.Scopes=$Scopes;$parameters.ContextScope='Process'
        ...
        'CertificateSubjectName' {$parameters.TenantId=$TenantId;$parameters.ClientId=$ClientId;$parameters.CertificateSubjectName=$CertificateSubjectName;$parameters.SendCertificateChain=$SendCertificateChain;$parameters.ContextScope='Process'}
        'ClientSecret' {$parameters.TenantId=$TenantId;$parameters.ClientSecretCredential=New-Object System.Management.Automation.PSCredential($ClientId,$ClientSecret);$parameters.ContextScope='Process'}
        'ManagedIdentity' {$parameters.Identity=$true;$parameters.ContextScope='Process';if($ClientId){$parameters.ClientId=$ClientId}}
    ...
    $sensitiveValues=@();$temporaryCredential=$null
    try {
