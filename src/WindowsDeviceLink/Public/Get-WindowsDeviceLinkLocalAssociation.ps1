function Get-WindowsDeviceLinkLocalAssociation {
    ...
    if (-not $linkRaw.Present -or -not $linkRaw.Bytes) {
        return [pscustomobject]@{
            ...
            JwtPresent                = $false
            JwtParseState             = 'Absent'
            JwtEncoding               = $null
            JwtTenantId               = $null
            JwtTenantClaim            = $null
            JwtDiscoveryUrl           = $null
            TenantSourcesMatch        = $null
            Source                    = 'Unavailable'
            TrustLevel                = 'Unavailable'
            SignatureValidation       = 'NotPerformed'
            ConflictDetected          = $false
            CloudChecked              = $false
            Conclusion                = 'The current DeviceLinkId is unavailable, so local tenant correlation cannot be performed.'
        }
    }
    ...
    try {
        ...
        $discoveryUrl = if (-not [string]::IsNullOrWhiteSpace($jwt.DiscoveryUrl)) {
            $jwt.DiscoveryUrl
        }
        else {
            $registryDiscoveryUrl
        }
        ...
        if ($linkRaw -and $linkRaw.Bytes) {
            [Array]::Clear($linkRaw.Bytes, 0, $linkRaw.Bytes.Length)
        }
        $linkRaw = $null
    }
}
