function Get-WindowsDeviceLinkAssociation {
    ...
    if ($Method -in @('DeviceCode','ClientSecret','EnvironmentVariable','AccessToken')) {
        if ($Environment -ne 'Global') {
            throw 'Native Device Association lookup currently supports the Global Microsoft cloud only.'
        }
        ...
        switch ($Method) {
            'DeviceCode' {
                ...
                Write-Information -InformationAction Continue -MessageData 'Using native OAuth device-code authentication for Device Association lookup.'
                $token = Get-WindowsDeviceLinkDeviceCodeToken @tokenParameters
                $nativeAccessToken = $token.AccessToken
            }
            'ClientSecret' {
                Write-Information -InformationAction Continue -MessageData 'Using native OAuth client-credentials authentication for Device Association lookup.'
            ...
            'EnvironmentVariable' {
                ...
                Write-Information -InformationAction Continue -MessageData 'Using native OAuth client-credentials authentication from environment variables for Device Association lookup.'
    ...
    try {
        if ($AssociationId) {
            ...
            if ($sdkMode) {
                $records = @(Invoke-MgGraphRequest -Method GET -Uri $uri -ErrorAction Stop)
            }
            else {
                $records = @(Invoke-RestMethod -Method GET -Uri $uri -Headers @{ Authorization = "Bearer $nativeAccessToken" } -ErrorAction Stop)
            }
        }
        else {
            ...
            if ($sdkMode) {
                $lookup = Invoke-MgGraphRequest -Method GET -Uri $uri -ErrorAction Stop
            }
            else {
                $lookup = Invoke-RestMethod -Method GET -Uri $uri -Headers @{ Authorization = "Bearer $nativeAccessToken" } -ErrorAction Stop
            }

            $records = @($lookup.value)
        ...
        foreach ($record in $records) {
            [pscustomobject]@{
                PSTypeName              = 'Windows.DeviceLink.Association'
                Id                      = $record.id
                TenantId                = $TenantId
                ...
                AssociationState        = $record.associationState
                ...
                DevicePreparationPolicyId = $record.devicePreparationPolicyId
