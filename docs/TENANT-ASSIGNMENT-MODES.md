...
| Backend | Operator selects the Function-provided target and chooses **Pre-register** or **Full register** | Can be unattended by calling `Set-WindowsDeviceLinkTenant` with an explicit target |
...
**Pre-register** or **Full register** actions; token acquisition happens as part of that action.
