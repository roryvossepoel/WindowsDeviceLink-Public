...
Device-side association completion is opt-in through `-Associate`.

With that switch, the supported path is:

`LocalOnly -> Register -> Preassociated -> Complete -> Associated`

Already-associated devices remain idempotent and report `AssociationResult = AlreadyAssociated` without invoking configure again.
